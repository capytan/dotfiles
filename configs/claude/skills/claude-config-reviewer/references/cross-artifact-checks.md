...
### 22. CLAUDE.md ↔ AGENTS.md Coexistence `[official]` (2026-09-24)
[Since v2.1.277 Claude Code reads AGENTS.md natively — but only when no CLAUDE.md-family file exists on the path, by default]
- "By default, Claude reads `AGENTS.md` only when you have no `CLAUDE.md` in your working directory or above it." Files that count: "a `CLAUDE.md`, `.claude/CLAUDE.md`, or `CLAUDE.local.md` in your working directory or any directory above it"; files that don't: "your `~/.claude/CLAUDE.md`, your organization's managed `CLAUDE.md`, and `.claude/rules/` files". "Because `CLAUDE.local.md` counts, adding one … stops Claude from reading `AGENTS.md` for you." (https://code.claude.com/docs/en/memory#agents-md, retrieved 2026-09-24)
- Detection: if the repo has an `AGENTS.md` **and** a CLAUDE.md-family file on the path, check that the CLAUDE.md either `@AGENTS.md`-imports it or restates what matters, or that **Project instructions** is `claude-md-and-agents-md` (user/`--settings`/managed `pluginConfigs["agents-md@builtin"]` only — the key is ignored in project/local settings). Otherwise AGENTS.md is silently unread → **Major** when AGENTS.md holds rules not present elsewhere. A CLAUDE.md that only points at AGENTS.md in prose, or a SessionStart hook that `cat`s AGENTS.md (duplicate copy once native reading applies) → **Minor** (claude-md anti-pattern "Telling Claude in Words to Read AGENTS.md"). `@AGENTS.md` import or `CLAUDE.md → AGENTS.md` symlink → pass.
- Subagents: natively-read AGENTS.md is part of the hierarchy that `omitClaudeMd` and Explore/Plan skip — extend Check 18 to AGENTS.md-dependent bodies.

### 23. Model-Era Instruction Sweep Across the Pool `[official]` (2026-09-24)
[Instructions that current models over-apply or refuse tend to be copied across CLAUDE.md, skills and agents; review them as one set so the fix is consistent]
- Sources (retrieved 2026-09-24): Opus 5 — remove generic verification / "double-check" / "use a subagent to verify" instructions and "don't think" rules; severity filters in review prompts are followed literally (https://platform.claude.com/docs/en/build-with-claude/prompt-engineering/prompting-claude-opus-5). Opus 5.5 (default Opus since Claude Code v2.1.280) — remove "think carefully before answering" lines and reasoning-in-response instructions (`reasoning_extraction`) (…/prompting-claude-opus-5-5). Fable 5 — "Audit existing skills and system prompts for reflection or show-your-thinking instructions" (…/prompting-claude-fable-5). Fable 5.1 — remove narration suppression and blanket anti-formatting (…/prompting-claude-fable-5-1). Anthropic blog: "we were overconstraining Claude Code, both through our system prompt and in our CLAUDE.md files and skills" (claude.dev/blog/the-new-rules-of-context-engineering-for-claude-5-generation-models).
- Detection (community method, wmedia.es `[community:mid]`): `grep -rniE "verify|double-check|re-check|think carefully|show (your )?(reasoning|thinking)|step by step|hold (all )?findings|no (markdown|bullets|headers)|only report (critical|high)|be conservative"` over `~/.claude/CLAUDE.md`, project CLAUDE.md/rules, `.claude/skills/`, `.claude/agents/`. Triage each hit against the exemptions in the per-type catalogs (concrete runnable checks, per-finding rationale, dedicated evaluator agents, category-scoped reviewers, official per-model sample blocks).
- Severity: any reasoning-reproduction instruction → **Major** (refusal risk) wherever it lives; the same overhead instruction duplicated in CLAUDE.md *and* a skill/agent → **Minor** (fix once at the most-shared layer). A global CLAUDE.md line that *contradicts* an agent/skill on these points (e.g. CLAUDE.md "don't add verification steps" vs an agent body "always re-verify") → **Minor** conflict.
...
| CLAUDE.md ↔ AGENTS.md Coexistence | Major when AGENTS.md-only rules are silently unread; Minor for prose pointer / SessionStart-print duplicate |
| Model-Era Instruction Sweep | Major for any reasoning-reproduction instruction; Minor for cross-layer duplication or contradiction of overhead instructions |
...
- 2026-09-24: Refreshed from pool research against code.claude.com/docs/en/{memory,skills,sub-agents,best-practices}, the platform per-model prompting pages (Opus 5, Opus 5.5, Fable 5, Fable 5.1), the Anthropic "new rules of context engineering for Claude 5" blog, and changelog v2.1.274–v2.1.281 (retrieved 2026-09-24). **Check 22 (new)**: CLAUDE.md ↔ AGENTS.md coexistence — native AGENTS.md reading (v2.1.277) is suppressed by any CLAUDE.md/.claude/CLAUDE.md/CLAUDE.local.md on the path unless imported or `claude-md-and-agents-md` is set in user/`--settings`/managed settings; Major when AGENTS.md-only rules go unread, Minor for prose pointers / SessionStart-print duplicates; Check 18 extended to AGENTS.md. **Check 23 (new)**: Model-era instruction sweep across CLAUDE.md + skills + agents with a grep recipe (wmedia.es method) and per-type exemptions; any reasoning-reproduction instruction Major (`reasoning_extraction` refusal on Fable 5/5.1 and Opus 5.5, the default Opus since v2.1.280), cross-layer duplication/contradiction Minor. Contents line and severity table updated. All existing checks re-verified; no severity changes. last_updated bumped to 2026-09-24.
