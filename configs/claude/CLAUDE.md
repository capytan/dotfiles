...
- validator は fail open、出力は `hookSpecificOutput.permissionDecision` schema。実装は `configs/claude/hooks/pretooluse-validate-command.sh`。除去アルゴリズムの詳細・過去の bypass 実例・変更時の注意は dotfiles の `.claude/rules/pretooluse-validator.md`
