...
- For heavy design/planning tasks, switch to `/model fable` before entering plan mode, and switch back to `/model opus` before the execution phase. Choose context clear when approving the plan (prompt cache is per-model, so this avoids re-writing the cache after the switch). Check the statusline model display to catch a forgotten switch-back
